 ...
 * ANSI_CONSOLE_TITLE_POST - Suffix for changing the window title, print the
 * window title in between
...
RLUTIL_INLINE void setBackgroundColor(int c) {
  ...
  SetConsoleTextAttribute(hConsole, (csbi.wAttributes & 0xFF0F) |
                                        (((WORD)c) << 4)); // Background colors
                                                           // take up the
                                                           // second-least
                                                           // significant byte
#else
